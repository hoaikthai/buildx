...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="snap-section flex flex-col justify-center bg-[var(--bg-secondary)]">
              ...
              {(['design', 'bim'] as const).map((key) => (
                <button
                  ...
                  className={`px-8 py-3 text-xs font-bold uppercase cursor-pointer transition-all duration-200 tracking-[3px] ${
                    tab === key ? 'bg-gold text-black' : 'bg-transparent text-[var(--text-muted)]'
                  }`}
                >
          ...
          {packages.map((pkg, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Header gradient */}
                  <div className="p-7 px-8" style={{background: 'linear-gradient(135deg, #f6d365 0%, #fda085 100%)'}}>
                    <p className="text-black/60 text-xs font-bold mb-1 tracking-[3px]">
                      {pkg.name}
                    </p>
                    <h3 className="text-black font-bold text-[1.3rem]">
                      {pkg.label}
                    </h3>
                    <div className="mt-4">
                      <span className="text-black font-bold text-[2rem]">{pkg.price}</span>
                  ...
                  <div className="p-6 px-8">
                    <p className="font-bold mb-4 text-[0.7rem] tracking-[3px] text-[var(--text-subtle)]">
